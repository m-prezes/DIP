...
def run_probing_with_intervention(
    ...
    save_results(
        ...
        aspect_names=config[f"{run['intervention']['aspect']}_aspect_names"],
        save_path=save_path,
    )
...
def run_experiment(
    run, config, model, tokenizer, label_prompt, confounding_prompt, tokens
):
    ...
    if run["intervention"]["aspect"] == "confounding":
        aspect_prompt = confounding_prompt
    elif run["intervention"]["aspect"] == "label":
